...
def litCricFriend(wordList, text):
    ...
    for words in cleanList:
        cleanerList.append(words.strip(string.punctuation))
    ...
    for word in wordList:
        if word in cleanerList:
            wordCount += cleanerList.count(word)
    ...
    return wordCount/len(cleanerList)
...
'''
Yes, both authors use the words 'a', 'an' and 'the' differently. Poe has a noticeably high
frequency for the words 'a', 'an', and 'the' compared to Neruda
'''
